...
def create_double_policy_agent(args, wm_model_args, wm_model, logger, maze_dim, num_actions, action_table):
    ...
    agent_low = LowLevelAgent(
        ...
        achievability_lr=args.achievability_lr,
        achievability_epochs=args.achievability_epochs,
    )
    ...
    agent_high = GoalAgent(
        ...
        entropy_coef=args.entropy_coef,
        target_entropy=args.target_entropy,
        mu_coef=0.001 if args.high_goal_output_delta else 0.0,
        goal_output_delta=args.high_goal_output_delta,
        goal_target_entropy=getattr(args, "high_goal_target_entropy", None),
        switch_target_entropy=getattr(args, "high_switch_target_entropy", 0.65),
        num_learning_epochs=args.high_policy_epochs,
        clip_param=args.high_policy_ppo_clip_param,
        exp_adv=getattr(args, "exp_adv", False),
    )
    agent = DoubleAgent(
        agent_high,
        ...
        wm_replay_capacity=args.wm_replay_capacity,
        wm_train_episodes=args.wm_train_episodes,
        wm_sfa_stability_coef=args.wm_sfa_stability_coef,
        wm_sensor_lp_reward_coef=args.wm_sensor_lp_reward_coef,
        wm_divergence_novelty_coef=args.wm_divergence_novelty_coef,
        wm_fixed=args.wm_fixed,
        high_warmup_goal_coef=getattr(args, "high_warmup_goal_coef", 0.005),
        ...
        achievability_warmup_updates=getattr(args, "achievability_warmup_updates", 300),
        ...
        high_achievability_max_kl=getattr(args, "high_achievability_max_kl", 0.05),
        high_achievability_lr_scale=args.high_achievability_lr_scale,
        high_achievability_policy_updates=args.high_achievability_policy_updates,
        high_achievability_overfit=args.high_achievability_overfit,
        virtual_horizon=getattr(args, "virtual_horizon", 3),
        high_virtual_goal_ppo=getattr(args, "high_virtual_goal_ppo", False),
        high_virtual_goal_imitation=getattr(args, "high_virtual_goal_imitation", False),
        high_virtual_goal_imitation_coef=getattr(args, "high_virtual_goal_imitation_coef", 1.0),
        high_virtual_goal_imitation_epochs=getattr(args, "high_virtual_goal_imitation_epochs", 1),
        high_virtual_goal_imitation_all_targets=getattr(args, "high_virtual_goal_imitation_all_targets", False),
        high_virtual_goal_imitation_fixed_dataset=getattr(args, "high_virtual_goal_imitation_fixed_dataset", False),
        high_virtual_goal_imitation_replay_capacity=getattr(args, "high_virtual_goal_imitation_replay_capacity", 0),
        high_virtual_goal_imitation_replay_samples=getattr(args, "high_virtual_goal_imitation_replay_samples", 64),
        high_virtual_goal_candidates=getattr(args, "high_virtual_goal_candidates", 4),
        high_virtual_goal_attempts=getattr(args, "high_virtual_goal_attempts", 2),
        high_virtual_goal_distance_penalty=getattr(args, "high_virtual_goal_distance_penalty", 0.1),
        low_fixed=getattr(args, "low_fixed", False),
        reset_high_agent_on_load=getattr(args, "reset_high_agent", False),
        sensor_max_bin=args.wm_sensor_max_bin,
        maze_dim=maze_dim,
        wm_weight_decay=args.wm_weight_decay,

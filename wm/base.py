...
class PredictionLossMixin:
    ...
    def _validate_obs_contract(self, obs, *, episode_start):
        ...
        if sensor is not None:
            sensor_latent = self.observation_encoder.encode(sensor)
        if not isinstance(sensor_latent, torch.Tensor) or sensor_latent.ndim != 3:
            raise ValueError("Observation encoder must return sensor_latent [B,T,D]")
        if sensor_latent.shape[-1] != self.observation_encoder.latent_dim:
            raise ValueError(
                f"Expected sensor latent dim {self.observation_encoder.latent_dim}, "
                f"got {sensor_latent.shape[-1]}"
            )
        state_shape = sensor_latent.shape[:2]
        ...
        if prev_actions is not None:
            ...
            if prev_actions.ndim != 3:
                raise ValueError(f"Expected prev_actions [B,T,A], got {tuple(prev_actions.shape)}")
    ...
    def compute_soft_target_loss(
        ...
        *,
        ...
        cfg = self.config
        del y_sensor_idx
        loss_sensor = self.observation_decoder.compute_loss(
            pred_sensor,
            y_sensor,
            key_padding_mask,
            cfg,
    ...
    def compute_losses(
        ...
        *,
        ...
        losses = self.compute_prediction_losses(
            ...
            y_sensor_idx=targets.get("y_sensor_idx"),
    ...
    def compute_predictions_metrics(
        ...
        *,
        ...
        pred_step: torch.Tensor,
        y_sensor,
        y_sensor_idx: Optional[torch.Tensor],
        y_loc_xy: torch.Tensor,
        y_turn: torch.Tensor,
        y_step: torch.Tensor,
        key_padding_mask: torch.Tensor,
        ...
        del aux_inputs, loc_max, sensor_min_idx, y_sensor_idx
        sensor_metrics = self.observation_decoder.compute_metrics(
            pred_sensor, y_sensor, key_padding_mask, self.config
        )
...
class DiscreteLatentPredictorBase(PredictionLossMixin, nn.Module):
    def __init__(
        ...
        *,
        ...
        if self.sensor_mode != "categorical":
            raise ValueError("Discrete maze world models require sensor_mode='categorical'")
    ...
    def compute_predictions_metrics(
        ...
        *,
        ...
        sensor_metrics = self.observation_decoder.compute_metrics(
            ...
            key_padding_mask,
            self.config,
            auxiliary_prediction=aux_inputs.get("prior_sensor_pred"),
        )
    ...
    def compute_prediction_losses(
        ...
        *,
        ...
        def compute_sensor_loss(decoder, sensor_prediction, target, mask):
            if sensor_prediction is None:
                return torch.tensor(0.0, device=pred_loc_x.device)
            return decoder.compute_loss(sensor_prediction, target, mask, cfg)

        def compute_prior_rollout_sensor_loss(prior_roll_sensor_pred, sensor_target):
            if prior_roll_sensor_pred is None:
                return torch.tensor(0.0, device=pred_loc_x.device)
            pred_len = self.observation_decoder.sequence_length(prior_roll_sensor_pred)
            target_raw = tree_map(sensor_target, lambda tensor: tensor[:, :pred_len])
            mask = key_padding_mask[:, :pred_len]
            return compute_sensor_loss(self.observation_decoder, prior_roll_sensor_pred, target_raw, mask)
        ...
        if aux_inputs is not None:
            ...
            if self.z_only_weight > 0 and z_only_pred is not None:
                z_only_sensor = compute_sensor_loss(
                    self.z_observation_decoder, z_only_pred, sensor_recon_target, key_padding_mask
                )
            ...
            if self.h_only_weight > 0 and h_only_pred is not None:
                h_only_sensor = compute_sensor_loss(
                    self.h_observation_decoder, h_only_pred, sensor_recon_target, key_padding_mask
                )
    ...
    def _decode_sensor_from_z(self, z_flat: torch.Tensor):
        ...
        if self.z_observation_decoder is None:
            return None
        return self.z_observation_decoder.decode(z_flat.detach())

    def _decode_sensor_from_h(self, h_flat: torch.Tensor):
        """Decode sensors from h only (auxiliary probe of deterministic state)."""
        if self.h_observation_decoder is None:
            return None
        return self.h_observation_decoder.decode(h_flat.detach())
    ...
    def _decode_feat(self, feat: torch.Tensor):
        ...
        return pred_sensor, loc_x, loc_y, heading_out, turn, step
